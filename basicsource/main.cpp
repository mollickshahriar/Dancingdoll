...
bool            gUseSmoothShading = true;           // whether to support the smooth shading model
...
int main(int argc, char** argv)
{
	...
	glutCreateWindow("FINAL PROJECT_L201826100121_MOLLICK SHAHRIAR HOSSAIN");				// comment out for initial full-screen
	...
	cout << "UNIVERSITY NAME: ZHEJIANG UNIVERSITY OF TECHNOLOGY"<<endl;
	cout << "COURSE NAME  : COMPUTER GRAPHIC"<<endl;
	cout << "TEACHER NAME : HUANG XIANPING  "<<endl;
	cout << "STUDENT NAME: MOLLICK SHAHRIAR HOSSAIN"<<endl;
	cout << "STUDENT ID  : L201826100121"<<endl;
	...
	// If we've got shader support, create some 3D noise for the shader use
	if (GLEW_VERSION_2_0)
	{
        glGenTextures(1, &gNoiseTexture3d);
        glBindTexture(GL_TEXTURE_3D, gNoiseTexture3d);
        CreateNoise3D();
    }
...
void drawScene(void)
{
	...
	if (gTextOn)
	{
	 ...
	 if(gCurrentFrameTime >= 1)
	 showCopyright("2021", "Warner Music", "Taxiride", "Counting Down the Days");
...
void keyPress(unsigned char key, int x, int y)
{
	switch (key)
	{
		...
		case 'a':
			...
			resetScene();						// initialise the objects in the scene
        ...
        // ---------------------------------------------------------------------------------
        // keypress 'D' - toggle diffuse/flat shading (only in default privateDraw)
        // ---------------------------------------------------------------------------------
        case 'D':
            gUseSmoothShading = !gUseSmoothShading;
            if (gProgramMode != kpmAnimating) glutPostRedisplay();
            break;
